extends Button
## The simple photo viewer
##
## Opens the Photos application
...
@onready var task: Button = $"../../Taskbar/PhotoGallery"
...
func _process(delta):
	pass
## Creates a new window meant for the Photos Application
func _on_button_press():
	...
	window.close_requested.connect(func(): window.queue_free(); self.disabled = false; self.text = "Photo Gallery"; task.position.y = 114)
	viewport.add_child(window)
	self.disabled = true
	self.text = self.text + " - Open"
	tween.tween_property(task, "position", Vector2(244.8, 31), 0.5)
