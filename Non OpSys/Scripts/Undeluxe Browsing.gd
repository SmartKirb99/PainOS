...
@onready var task: Button = $"../../Taskbar/UndeluxeBrowser"
...
func _on_button_press():
	...
	window.close_requested.connect(func(): window.queue_free(); self.disabled = false; self.text = "Undeluxe Browsing"; task.position.y = 114)
	viewport.add_child(window)
	self.disabled = true
	self.text = self.text + " - Open"
	tween.tween_property(task, "position", Vector2(81.6, 31), 0.5)
